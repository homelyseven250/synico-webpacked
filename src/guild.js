...
document.addEventListener('DOMContentLoaded', function (e) {
    ...
    document.getElementById('ticket-submit').addEventListener('click', e => {
        fetch(`/api/guild/${location.href.substring(location.href.lastIndexOf('/') + 1)}/ticket/message`,
            {
                method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(
                    {
                        message: document.getElementById('ticket-message').innerText
                    }
                )
            }).then(res => res.status == 200 ? alert('Success') : alert('Error'))
    });
